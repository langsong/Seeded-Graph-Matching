...
from FeatureEngineering.data import *
from FeatureEngineering.model import train_random_forest
...
def compare_seeding(graph_gen_func, seeding_funcs_list, algorithm, seed_nums_list=SEED_COUNTS, n_trials=TRIALS_PER_SEED_NUMBER):
...
def seed_quality_experiment(metric, n_trials=200, n_seeds=10, graph_generator=gen_ER_graphs
):
    """
...
if __name__ == "__main__":
    ...
    # res = compare_seeding(gen_PAPER_graphs, [random_seeds,jaccard_cluster_seeds,neighbor_degree_seeds,highest_degree_seeds,betweenness_seeds],graph_match_percolation,
    #                 SEED_COUNTS)
    # plot_results(res,SEED_COUNTS)


    # df = generate_feature_dataset(
    #     graph_gen_funcs=[gen_ER_graphs],
    #     seeding_funcs=[random_seeds],
    #     seed_nums=SEED_COUNTS,
    #     algorithms=[graph_match_percolation],
    #     n_trials=500)
    # model, metrics, importance, predictions = train_random_forest(df)
    # joblib.dump(model, "rf_seed_model.pkl")
    # print(f"R² = {metrics['r2']:.4f}")
    # print(f"MSE = {metrics['mse']:.4f}")
    # print(importance.head(20))
    # print('got it')
    # rf_random_seeds = RFRandomSearch(model)
    # RF_MODEL = model
    # models = []
    ...
    #     n_trials=100)
    #     model, metrics, importance, predictions = train_random_forest(df)
    #     rf_random_seeds = RFRandomSearch(models)
    #     models.append(model)
    #     print(f"R² = {metrics['r2']:.4f}")
    #     print(f"MSE = {metrics['mse']:.4f}")
    # joblib.dump(models, "rf_seed_models.pkl")
    # models = joblib.load("rf_seed_models.pkl")
    # rf_random_seeds = RFRandomSearch(models)
    res = compare_seeding(gen_ER_graphs,[random_seeds,rf_random_seeds, rf_greedy_seeds],graph_match_percolation)
    plot_results(res)